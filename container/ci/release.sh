...
let {ARTIFACT_NAME, log, error, github, run} = require('./ci-helpers');
...
(function main() {
  ...
  if (releaseNeeded()) {
    ...
    github.get(`releases/tags/${release.tag_name}`, 'build/release.json');
  }
})();

function verifyCleanWorkingDirectory() {
  let resp = run('git', 'status', '--porcelain').stdout.toString();
  if (resp) error(`workspace is not clean:\n\n${resp}`);
}

function createRelease() {
  let tag = tagReleaseWith(nextTag());
  return github.post('releases', {'tag_name': tag.ref.split('/').pop(), 'target_commitish': tag.object.sha}).json;
}

function tagReleaseWith(tag) {
  let sha = run('git', 'rev-parse', 'HEAD').stdout.toString().trim();
  log(`tagging ${sha} with ${tag}`);
  return github.post('git/refs', {ref: `refs/tags/${tag}`, sha}).json;
}
...
function nextTag() {
  ...
  try {
    let currentTag = github.get('releases/latest').json.tag_name;
...
function uploadAssets(release) {
  ...
  github.upload(`${release.upload_url.replace(/{.*}/, '')}?name=${ARTIFACT_NAME}.tar.gz&label=${ARTIFACT_NAME}`, 'application/x-gzip', `build/${ARTIFACT_NAME}.tar.gz`);
  return github.upload(`${release.upload_url.replace(/{.*}/, '')}?name=${ARTIFACT_NAME}.md5&label=${ARTIFACT_NAME}.md5`, 'text/plain', 'build/slug.md5');
}
...
function getCurrentChecksum() {
  let latestRelease = github.get('releases/latest').json;
  let checksumAsset = latestRelease.assets.find((asset) => asset.name === `${ARTIFACT_NAME}.md5`);
  if (!checksumAsset) return false;
  return run('cat', github.download(checksumAsset.url)).stdout.toString().trim();
}
